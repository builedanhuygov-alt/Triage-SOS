...
import { Building2, Siren, Activity, Clock, ShieldAlert, Radio } from "lucide-react";
...
/** TopBar: nav glass + KPIs + Emergency Override. */
export default function TopBar({ override, setOverride, patientCount, waiting }) {
  return (
    <div className="space-y-3">
      <div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl bg-white/[0.04] p-4 ring-1 ring-white/10 backdrop-blur-xl">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            <p className="flex items-center gap-1 text-[11px] text-zinc-400"><Radio className="h-3 w-3 text-emerald-400" /> Live via Socket.io • {patientCount} ca radar</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-3">
        ...
        <Kpi icon={Clock} label="Chờ trung bình" value={`${waiting} phút`} sub="Mục tiêu < 10 phút" accent="text-cyan-300" />
      </div>
    </div>
  );
}
